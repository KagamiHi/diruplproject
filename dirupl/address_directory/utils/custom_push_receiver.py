...
class PushReceiver:
  ...
  def login(self):
    ...
    self.send(req)
    login_response = self.recv(first=True)
  ...
  def status_check(self, term):
    time.sleep(term)
    self.socket.shutdown(socket.SHUT_RDWR)
    self.socket.close()


  def listen(self, callback, term, obj=None):
    """
    ...
    self.checkin_thread = Thread(target = self.status_check, args=(term,), daemon = True)
